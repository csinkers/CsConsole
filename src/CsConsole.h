...
public:
	...
	std::string Arg(const std::string& name)
	{
		...
		if (!opt)
			throw ConsoleCommandException("Expected parameter \"" + name + "\"");
...
public:
	...
	[[nodiscard]] std::string ShortDescription() const override { return ""; }
...
public:
	...
	{
		return "When given a command line, shows detailed info on the command. When run without an argument, lists all available commands.";
	}
	...
	void DescribeCommand(const std::string& name, IConsoleOutput& o) const
	{
		...
		if (!m_parser.TryGetCommand(name, command) || !command)
			throw ConsoleCommandException("Unknown command \"" + name + "\"");
	...
	void Add(const std::shared_ptr<ICommand>& command)
	{
		...
		for (const auto& alias : command->Names())
		{
			if (m_commands.contains(alias))
				throw ConsoleCommandException("Could not register alias \"" + alias + "\" for command as it is already registered");
		}
	...
	void Handle(const std::vector<std::string>& args, IConsoleOutput& o, TState& state)
	{
		std::shared_ptr<ICommand> command;
		{
			...
			if (args.empty() || !m_commands.contains(args[0]))
				throw ConsoleCommandException("Unknown command \"" + (args.empty() ? "" : args[0]) + "\"");
		...
		throw ConsoleCommandException("Command does not support invocation");
	}
};
